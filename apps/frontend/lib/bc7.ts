...
function write4x4(a: Uint8Array, w: number, h: number, sx: number, sy: number, b: Uint8Array) {
  for (let y = 0; y < 4; y++) {
    if (sy + y >= h) continue;
  ...
  for (let y = 0; y < height; y += 4) {
    for (let x = 0; x < width; x += 4) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < pnts; j++) rawClr[i * pnts + j] = readBits(data, pos, len);
      }

      for (let j = 0; j < pnts; j++) {
        if (mode == 1 && (j & 1) == 1) pos.boff--; // Ps shared per subset
        const bit = readBits(data, pos, plen);
        for (let i = 0; i < 3; i++) rawClr[i * pnts + j] = (rawClr[i * pnts + j] << plen) | bit;
        if (alen != 0) rawClr[3 * pnts + j] = (rawClr[3 * pnts + j] << plen) | bit;
      }
      clen += plen;
      if (alen != 0) alen += plen;
      ...
      for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < pnts; j++) clr[i * pnts + j] = rawClr[i * pnts + j] * cf;
      }
      ...
      for (let i = 0; i < 64; i += 4) {
        ...
        const r = (1 - f) * clr[0 * pnts + 2 * ss + 0] + f * clr[0 * pnts + 2 * ss + 1];
        const g = (1 - f) * clr[1 * pnts + 2 * ss + 0] + f * clr[1 * pnts + 2 * ss + 1];
        const b = (1 - f) * clr[2 * pnts + 2 * ss + 0] + f * clr[2 * pnts + 2 * ss + 1];
        const a = (1 - f) * clr[3 * pnts + 2 * ss + 0] + f * clr[3 * pnts + 2 * ss + 1];
